...
use common::{
    ...
    spawn_genmcp_websocket, wait_for_tcp_connect, write_temp_config,
};
...
async fn websocket_works_without_auth_when_jwt_not_enabled() {
    ...
    let _server = spawn_genmcp_websocket(&cfg.path, "127.0.0.1", port, None);
...
async fn websocket_auth_enabled_rejects_missing_authorization_header() {
    ...
    let _server = spawn_genmcp_websocket(&cfg.path, "127.0.0.1", port, Some(&secret));
