...
impl JwksVerifier {
    ...
    pub async fn from_oidc_issuer(issuer_url: &str) -> Result<Self> {
        ...
        let client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| TransportError::Authentication(format!("Failed to create HTTP client: {}", e)))?;
        
        let config: OidcConfig = client
            .get(&well_known_url)
            .send()
            .await
            .map_err(|e| TransportError::Authentication(format!("Failed to fetch OIDC config: {}", e)))?
            .error_for_status()
            .map_err(|e| TransportError::Authentication(format!("OIDC config request failed: {}", e)))?
            .json()
            .await
            .map_err(|e| TransportError::Authentication(format!("Failed to parse OIDC config: {}", e)))?;
    ...
    pub async fn verify(&self, token: &str) -> Result<jwtk::HeaderAndClaims<serde_json::Value>> {
        ...
        verifier.verify(token)
            .await
            .map_err(|e| TransportError::Authentication(format!("JWT verification failed: {}", e)).into())
    }
}
